        ...
        try {
            ...
            if (req.body.action) {
                ...
                if (req.body.action === 'thong_qua') {
                    await db.none(`
                        insert into "submit_history" (viewer, email, phonenumber, editor, leader_submit_date, report, leader)
                        values ($1, $2, $3, $4, $5, $6, $7)
                    `, [viewer.name, viewer.email, viewer.phonenumber, editor.email, date, viewer.report, req.user.email])
                    await db.none(`
                        delete from "report"
                        where email = $1;
                    `, [viewer.email])
